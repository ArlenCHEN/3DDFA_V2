        ...
        for exp in expression_list:
            ...
            with open(split_file, 'a') as f:
                for level in level_list:
                    ...
                    len_video_clip_list = len(video_clip_list)
                    valid_len = int(len_video_clip_list/3)
                    video_clip_list = video_clip_list[:valid_len] # Only extract data from the first video clip
                    for clip in tqdm(video_clip_list):
                        ...
                        for i, frame in tqdm(enumerate(frames)):
                            ...
                            if ref_img_2d_landmarks is None or np.any(ref_img_2d_landmarks<0):
                                continue
                            ...
                            if new_ref_img_2d_landmarks is None or np.any(new_ref_img_2d_landmarks<0):
                                continue
                            ...
                            for target in str_temporal_num_list: # Loop over the next few target frames
                                ...
                                mask = -1*np.ones((new_target_gt_img.shape[0], new_target_gt_img.shape[1]))
                                ...
                                mask[target_gt_left_coords[1]-delta:target_gt_left_coords[0]+delta, target_gt_left_coords[3]-delta:target_gt_left_coords[2]+delta] = 1
                                ...
                                mask[target_gt_right_coords[1]-delta:target_gt_right_coords[0]+delta, target_gt_right_coords[3]-delta:target_gt_right_coords[2]+delta] = 2
                                ...
                                mask[target_gt_mouth_coords[1]-int(2*delta):target_gt_mouth_coords[0]+int(4.3*delta), target_gt_mouth_coords[3]-int(3.5*delta):target_gt_mouth_coords[2]+int(3.5*delta)] = 3
                                ...
                                if target_left_img_2d_landmarks is None or np.any(target_left_img_2d_landmarks<0): # It is possible to not detect landmarks from the left image
                                    continue
                                ...
                                if target_right_img_2d_landmarks is None or np.any(target_right_img_2d_landmarks<0): # It is possible to not detect landmarks from the right image
                                    continue
                                ...
                                if target_top_img_2d_landmarks is None or np.any(target_top_img_2d_landmarks<0):
                                    continue
